...
public class MinimumTimeRequired {
	static long minTime1(long[] machines, long goal) {
		long produces = 0;
		long time = 0;
		long lcm = lcm(machines);
		long produceInLcmTime = 0;
		for (int i = 0; i < machines.length; i++) {
			produceInLcmTime+= lcm/machines[i];
		}
		while(produces+produceInLcmTime<goal)
		{
			produces+=produceInLcmTime;
			time+=lcm;
		}
		while (produces < goal) {
			time++;
	...
	private static long lcm(long[] myArray) {
		long min, max, x, lcm = 0;
	...
	static long minTime5(long[] machines, long goal) {
		long min = Integer.MAX_VALUE;
		long max = Integer.MIN_VALUE;
		for (int i = 0; i < machines.length; i++) {
			min = Long.min(min, machines[i]);
			max = Long.max(min, machines[i]);
		}
		long minTime  = min*goal/machines.length;
		long maxTime = max*goal/machines.length;
		
		return findTime(machines, minTime, maxTime, goal);
	}
	
	

	private static long findTime(long[] machines, long minTime, long maxTime, long goal) {
		long mid = minTime+maxTime;
		mid/=2;
		long produce = 0;
		for (int i = 0; i < machines.length; i++)
		...
		if(produce==goal)
			return mid;
		else if(produce>goal)
			return findTime(machines, minTime, mid, goal);
		else
			return findTime(machines, mid, maxTime, goal);
	}
